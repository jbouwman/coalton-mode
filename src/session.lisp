...
(defun read-message (session)
  (handler-case
    ...
    (sb-int:closed-stream-error ()
      (format t ";; Remote session disconnected (stream closed)~%")
      nil)
    (end-of-file ()
      (format t ";; Remote session disconnected (end of file)~%")
      nil)))
...
(define-handler ("initialize" initialize-params)
  ...
  (method-not-found request "Intentional failure in initialize."))
      ...
      (cond ((not message-class)
            ...
            (t
             (funcall message-handler
                      session
                      request
                      (new-message message-class
                                   (get-field request :params))))))))
...
(defmethod stop ((session session))
  (format t ";; Stopping session ~a~%" session) ; FIXME logging
