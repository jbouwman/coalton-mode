...
(defun copy-message-field (message-field)
  (with-slots (name json class optional vector) message-field
  ...
  (let ((field (%get-field (message-class message) key)))
    (make-message (name (message-class message))
                  (acons (json-key field)
                         (accept-p (message-class field) value) ; misnomer
